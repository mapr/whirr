...
public class MapRConfigurationBuilderTest {
  ...
  public void setUp() throws Exception {
    ...
    defaults.addProperty("mapr-core.p1", "core1");
    defaults.addProperty("mapr-core.p2", "core2");
    defaults.addProperty("mapr-mfs.p1", "mfs1");
    defaults.addProperty("mapr-mapreduce.p1", "mapred1");
...
//        ImmutableSet.of(MapRMRMasterClusterActionHandler.ROLE),
        ImmutableSet.of(MapRZookeeperClusterActionHandler.ROLE,
            MapRCLDBClusterActionHandler.ROLE,
            MapRJobTrackerClusterActionHandler.ROLE,
            MapRFileServerClusterActionHandler.ROLE,
            MapRTaskTrackerClusterActionHandler.ROLE),
            "10.0.0.1", "10.0.0.1", "1", node, fakeDnsResolver);
    instances.add(master);
    for (int i = 0; i < numberOfWorkers; i++) {
      int id = i + 2;
      instances.add(new Instance(new Credentials("", ""),
        ImmutableSet.of(MapRFileServerClusterActionHandler.ROLE,
              MapRTaskTrackerClusterActionHandler.ROLE),
  ...
  public void testCommon() throws Exception {
    Configuration conf = MapRConfigurationBuilder.buildCommonConfiguration(
        clusterSpec, cluster, defaults);
             // # of "mapr-core" attributes in default config created above
    assertThat(Iterators.size(conf.getKeys()), is(2));
    assertThat(conf.getString("p1"), is("core1"));
    assertThat(conf.getString("p2"), is("core2"));
//    assertThat(conf.getString("fs.default.name"), matches("hdfs://.+:8020/"));
  ...
  public void testOverrides() throws Exception {
    ...
    overrides.addProperty("mapr-core.p1", "overridden1");
    overrides.addProperty("mapr-core.p2", "overridden2");
    overrides.addProperty("mapr-core.fs.default.name", "not-overridden");
...
/* No need to test HDFS configuration details for MapR */
/*
  @Test
  public void testHdfs() throws Exception {
    Configuration conf = MapRConfigurationBuilder.buildHdfsConfiguration(
        clusterSpec, cluster, defaults,
...
*/
  ...
  public void testMapReduce() throws Exception {
    ...
    Configuration conf = MapRConfigurationBuilder
    ...
    assertThat(conf.getString("mapred.reduce.tasks"), is("18"));
    assertThat(conf.getString("mapred.local.dir"),
        is("/data0/mapred/local,/data1/mapred/local"));
  }

  @Test
  public void testOverridesNumberOfMappers() throws Exception {
    Configuration overrides = new PropertiesConfiguration();
    overrides.addProperty("mapr-mapreduce.mapred.tasktracker.map.tasks.maximum", "70");
    clusterSpec = ClusterSpec.withNoDefaults(overrides);
    Configuration conf = MapRConfigurationBuilder.buildMapReduceConfiguration(
        clusterSpec, cluster, defaults,
        Sets.newLinkedHashSet(Lists.newArrayList("/data0", "/data1")));
  ...
  public void testOverridesNumberOfReducers() throws Exception {
    Configuration overrides = new PropertiesConfiguration();
    overrides.addProperty("mapr-mapreduce.mapred.reduce.tasks", "7");
    clusterSpec = ClusterSpec.withNoDefaults(overrides);
    Configuration conf = MapRConfigurationBuilder.buildMapReduceConfiguration(
        clusterSpec, cluster, defaults,
        Sets.newLinkedHashSet(Lists.newArrayList("/data0", "/data1")));
